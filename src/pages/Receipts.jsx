...
export default function Receipts() {
  ...
  const { data: receipts = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select(
          `*,
          students ( id, first_name, last_name, admission_no, medium_id ),
  ...
  const filteredReceipts = useMemo(() => {
    ...
    if (filters.start_date) {
      result = result.filter((r) => r.receipt_date >= filters.start_date);
    }
    if (filters.end_date) {
      result = result.filter((r) => r.receipt_date <= filters.end_date);
    }

    // Filter by student ID (explicit selection)
    if (filters.student_id) {
      result = result.filter((r) => r.students?.id == filters.student_id);
    }

    // Filter by student set from course/batch/medium
  ...
  // Students filtered by course/batch/medium for the student dropdown
