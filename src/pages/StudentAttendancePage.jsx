...
export default function StudentAttendancePage() {
  ...
  const { data: sessions = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const markMap = {};
      marks?.forEach((m) => { markMap[m.session_id] = m.status; });
  ...
  return (
    <AdminLayout>
      <h1 className="text-3xl font-righteous text-primary-dark mb-4">My Attendance</h1>
      <div className="bg-white rounded-xl p-5 shadow-sm border border-secondary-light mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-secondary mt-1">{presentCount} present / {total} sessions</p>
        </div>
      </div>
          ...
          <tbody>
            {sessions.map((s) => (
              <tr key={s.id} className="border-t">
                ...
                <td>
                  {s.status === "Present" ? (
                    <span className="text-green-600 flex items-center gap-1"><CheckCircle size={16} /> Present</span>
                  ) : (
                    <span className="text-red-600 flex items-center gap-1"><XCircle size={16} /> Absent</span>
                  )}
                </td>
              </tr>
            ))}
