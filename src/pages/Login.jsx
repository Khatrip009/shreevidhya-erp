...
export default function Login() {
  ...
  const { user, profile, loading: authLoading, orgAccessDenied } = useAuth();
    // ── Fetch the correct theme directly (org 3) ──
  ...
  if (authLoading || (user && !profile)) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-secondary-bg">
        <Text className="text-secondary" style={{ fontFamily: bodyFont }}>
      ...
      <Card className="w-full max-w-md shadow-xl" variant="borderless">
        <Space orientation="vertical" size="large" className="w-full">
          ...
          <Title
            ...
            style={{ fontSize: "1rem", fontFamily: headingFont, color: primaryColor }}
          >
            {orgName}
          </Title>
          ...
          >
            {org?.tagline || "Empowering Education"}
          </Text>

          <Text
            className="text-center block"
            style={{ fontFamily: bodyFont, color: "#555" }}
          >
          ...
          <Text
            ...
            style={{ fontFamily: bodyFont, color: "#999" }}
          >
